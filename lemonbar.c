...
const char *fragmentShaderSource = "#version 130\n"
    ...
    "out vec4 FragColor;\n"
    "\n"
    "// Put custom local shader functions somewhere here:\n"
    ...
    "}\n"
    "\n"
    "// This is close to how the ShaderToy wrapper works. Modify this!\n" 
    "void mainImage( out vec4 fragColor, in vec2 fragCoord ) {\n"
    //"// this is kept for easy copying until I get file loading working\n"
    //"   ;\n"
    ...
    //"   fragColor = vec4(0.0f, 1.0f, 0.0f, 1.0f);\n"
    //"   fragColor = vec4(Color, 1.0f);\n"
    "   vec3 mixerColor = hsv2rgb_smooth(vec3(iGlobalTime, 1.0, 1.0));\n"
    "   vec2 tv = vec2(Texcoord.x + (circle/80.0), Texcoord.y);\n"
    "   fragColor = mix(texture(barTex, tv), vec4(mixerColor, 1.0f), 0.5);\n"
    //"   fragColor = texture(barTex, tv);\n"
    "// This is the 'correct' way to display the standard lemon bar text:\n"
    "//   fragColor = texture(barTex, Texcoord); // Draw bar without modifications\n" 
    "}\n"
    "// This main() function calls mainImage() like ShaderToys. Don't change it...\n"
...
main (int argc, char **argv)
{
    ...
    for (;;) {
        ...
        // Copy the original lemonbar pixel map output into a GL texture if it's been updated:
        if (redraw) {
          for (monitor_t *mon = monhead; mon; mon = mon->next) {
              ...
              pixmap_height = bh;
              printf("monitor:%i, w:%i, h:%i\n", mon->window, pixmap_width, pixmap_height);
          }
